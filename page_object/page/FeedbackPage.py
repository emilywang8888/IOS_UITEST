...
class Feedback(BasePage):
    ...
    _add_pic = '//*[contains(@type,"TypeImage")]/..'  # 添加图片的+号地址
    ...
    _album_list = '//*[contains(@name,"月")]'  # 相册列表
    ...
    # 添加图片的+号地址也用xpath定位组元素，和相册一样，每添加一张图片，+号右移动一个位置
    def add_feedback_pic(self, index):
        for i in range(index):
            ...
            #     time.sleep(2)
            pic = self.find_element_ablum(self._add_pic)
            pic[i].click()
